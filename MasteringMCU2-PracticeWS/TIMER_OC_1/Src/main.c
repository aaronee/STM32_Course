 ...
 *  GOAL: using general timer and Output Compare to generate the desired square waveform
 *  	500HZ, 1KHZ, 2KHZ, 4Khz through out 4 different output channels
...
void Timer2_Init(void)
{
	...
	if (HAL_TIM_OC_Init(&htimer2) != HAL_OK)	// Enable timer input capture time based
	{
		Error_handler();
	}
	...
	timer2OC_Config.OCPolarity = TIM_OCPOLARITY_HIGH; // Chap9vid3 - 10m00s
