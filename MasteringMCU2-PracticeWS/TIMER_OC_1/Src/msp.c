...
{
	//Enable CLK for TIMER and it GPIO
	__HAL_RCC_TIM2_CLK_ENABLE();
	__HAL_RCC_GPIOA_CLK_ENABLE();
	//Set up GPIO output param
	GPIO_InitTypeDef timer2IO_Init;
	timer2IO_Init.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3;
	timer2IO_Init.Mode = GPIO_MODE_AF_PP;
	timer2IO_Init.Alternate = GPIO_AF1_TIM2;
	HAL_GPIO_Init(GPIOA,&timer2IO_Init);
