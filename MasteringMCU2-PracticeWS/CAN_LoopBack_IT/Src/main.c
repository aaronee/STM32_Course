 ...
 *      Author: khoahuynh
 *      Chap16vid1 -> Chap16vid3
 *      Testing IRQs of CAN
 *      Sending 2 messages, notify when the messages are sent using IRQ
 *      Notify when RX FIFO0 is in pending state(CAN RX received message) usinging IRQ
 *      Implement Bus_FAIL notification IRQ when there is error on bus which cause bus_fail
 */
#include "main.h"
#include <string.h>
#include <stdio.h>
...
uint8_t mesg2[20] = {'X','i','n','C','h','a','o'};
...
int main(void)
{
	...
	{
		Error_handler(); // move CAN bus to NORMAL mode from INITIALIZE mode
	}
...
void CAN1_Tx(uint8_t mesg[], uint32_t mailbox)
{
	...
	if (HAL_CAN_AddTxMessage(&hcan1,&TxHeader,mesg2, &mailbox) != HAL_OK)
	{
		Error_handler();
	}
...
void HAL_CAN_TxMailbox0CompleteCallback(CAN_HandleTypeDef *hcan)// Tx mailbox empty
{
	sprintf((char *)mesg,"CAN transmitted - Mailbox 0 \r\n");
	if (HAL_UART_Transmit(&huart3,mesg,strlen((char *)mesg),HAL_MAX_DELAY) != HAL_OK)
	{
		Error_handler();
	}
}
void HAL_CAN_TxMailbox1CompleteCallback(CAN_HandleTypeDef *hcan)// Tx mailbox empty
{
	sprintf((char *)mesg,"CAN transmitted - Mailbox 1 \r\n");
	if (HAL_UART_Transmit(&huart3,mesg,strlen((char *)mesg),HAL_MAX_DELAY) != HAL_OK)
	{
		Error_handler();
	}
}
void HAL_CAN_TxMailbox2CompleteCallback(CAN_HandleTypeDef *hcan)// Tx mailbox empty
{
	sprintf((char *)mesg,"CAN transmitted - Mailbox 2	 \r\n");
	if (HAL_UART_Transmit(&huart3,mesg,strlen((char *)mesg),HAL_MAX_DELAY) != HAL_OK)
	{
		Error_handler();
	}
}
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)// FIFO 0 mesg pending
{
...
void HAL_CAN_ErrorCallback(CAN_HandleTypeDef *hcan) // bus_off
{
	sprintf((char *)mesg,"CAN ERROR \r\n");
	if (HAL_UART_Transmit(&huart3,mesg,strlen((char *)mesg),HAL_MAX_DELAY) != HAL_OK)
	{
		Error_handler();
	}
