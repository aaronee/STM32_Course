 ...
 *
 * Description: configure Alarm A to go off at 23:15:30 everyday
 * 				Set up calendar and alarm by pressing B1 button
 * 				Use Alarm interrupt as wake up source
 *
 */
...
void RTC_Init(void)
{
	/*! <no checking, allow rtc to be set whenever this function is called>*/
...
void RTC_CalendarConfig(void)
{
	/*! <no checking, allow calendar to be set whenever this function is called>*/
		...
		RTCtime_param.Hours = 23; // this is BIN format
		RTCtime_param.Minutes = 15;
		RTCtime_param.Seconds = 20;
...
void RTC_AlarmConfig(void)
{
	...
	RTCAlarm_param.AlarmTime.Hours = 23;
	RTCAlarm_param.AlarmTime.Minutes = 15;
	RTCAlarm_param.AlarmTime.Seconds = 30;
	RTCAlarm_param.AlarmMask = RTC_ALARMMASK_DATEWEEKDAY;
	...
	sprintf(mesg,"Alarm A is set 23:15:30 \r\n");
	if (HAL_UART_Transmit(&hlpuart1,(uint8_t *)mesg,strlen(mesg),HAL_MAX_DELAY) != HAL_OK)
	{
		Error_handler();
	}
