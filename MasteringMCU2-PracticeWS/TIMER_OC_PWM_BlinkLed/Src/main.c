 ...
 *  GOAL: using timer15 to control led through PWM
 *  	Dimming-glowing-period = 1s
 *  	PWM period = 10ms = 100Hz
 *  	Adjust 100 times PWM period to archive 1s Dimming-glowing-period
 *  	Chap10vid-Chap10vid10
 */
...
uint32_t brightness = 0; //set desired brightness value
uint8_t sw = 1; // glow if = 1, dim if = 0
...
int main(void)
{
	...
	Timer15_Init();
	if (HAL_TIM_PWM_Start_IT(&htimer15,TIM_CHANNEL_1) != HAL_OK)
	{
		Error_handler();
	}
	while(1);
...
void Timer15_Init(void)
{
	...
	htimer15.Init.Prescaler = 4999; //10Khz
	htimer15.Init.Period = 99;//10ms period
	if (HAL_TIM_PWM_Init(&htimer15) != HAL_OK)	// Enable timer input capture time based
	{
		Error_handler();
	}
	...
	timer15OC_Config.OCMode = TIM_OCMODE_PWM1;
	timer15OC_Config.Pulse = brightness; // 1s trigger with 1Khz timer count frequency
	timer15OC_Config.OCPolarity = TIM_OCPOLARITY_HIGH;
	if (HAL_TIM_PWM_ConfigChannel(&htimer15,&timer15OC_Config,TIM_CHANNEL_1) != HAL_OK)
	{
		Error_handler();
	}
}
void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim)
{
	if (sw == 1)
	{
		...
		if (brightness >= 99) // increase 100 times to make dimming period = 1s
		{
			sw = 0;
		}
	}
