...
void HAL_RTC_MspInit(RTC_HandleTypeDef *hrtc)
{
	...
	{
		// already enable LSE in SYSTEM CLK INIT
		// Select LSE as RTC clk
		RCC_PeriphCLKInitTypeDef RTCCLK_param = {0};
		RTCCLK_param.PeriphClockSelection = RCC_PERIPHCLK_RTC;
		RTCCLK_param.RTCClockSelection = RCC_RTCCLKSOURCE_LSE;
		if (HAL_RCCEx_PeriphCLKConfig(&RTCCLK_param) != HAL_OK)
		{
			Error_handler();
		}
		// Enable clk RTC
