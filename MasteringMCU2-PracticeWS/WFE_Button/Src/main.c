 ...
 *      	Using WFE instruction to put processor into sleep when needed
 *      	Chap20vid5 to Chap20vid11
 *
 *      	1/ The event is different with interrupt (which will have IRQ, stack/unstack process)
 *      	2/ The event bit can be set using a EXTI line, WFE will always wake up if event bit is set
 *      	3/ WFE and WFI all wake up processor with interrupt
 *      	4/ SEVONPEND 0 or 1 does effect on event to wake up by WFE mode
 *      		SEVONPEND only suspend low priority interrupt to wake up the processor
 */
...
int main(void)
{
	...
	while(1)
	{
		...
		SysTick->CTRL = 0x0;
		/* Systick generate interrupt every 10ms, always set event bit to 1,
		 * and woke up processor right after it go to sleep, so we have
		 * to turn it of before sleep*/
		SCB->ICSR |= (1<<25);
		/* Clear the Pending bit of Systick timer*/
		__WFE(); // go to sleep mode if no event are set
		/*The processor wakes up because PC13 is set the event bit to 1
		 * PC13 is set to GPIO_MODE_EVT_FALLING
		 * */
		SysTick->CTRL = 0x1;
	}
