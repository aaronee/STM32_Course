 ...
 *
 *      Goal: DMA with UART
 *      DMA transfer 10 byte of data from UART rx DR to buffer(which are allocated automatically
 *      on runtime), after write 10 byte, dma IRQ trigger and uart rx complete call back also
 *      been called as the RX has finished receive 10 bytes of data
 */
