 ...
 *
 *      Goal: DMA with ADC
 *      config ADC to sample data from temperature sensor with 12 bit configuration.
 *      config ADC to signal DMA to transfer the data from DR to SRAM in haft-word (because sample data is 12bit)
 *      THIS EXAMPLE HAVE NOT BEEN IMPLEMENETED AS LACKING OF TEMPERATURE SENSOR
 */
...
void DMA_Init(void)
{
	...
	hDMA1_P2M_stream.Instance = DMA1_Channel1;
	hDMA1_P2M_stream.Init.Direction = DMA_PERIPH_TO_MEMORY;
	hDMA1_P2M_stream.Init.PeriphInc = DMA_PINC_DISABLE;
	hDMA1_P2M_stream.Init.MemInc = DMA_MINC_ENABLE;
	hDMA1_P2M_stream.Init.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
	hDMA1_P2M_stream.Init.MemDataAlignment = DMA_MDATAALIGN_BYTE;
	hDMA1_P2M_stream.Init.Mode = DMA_NORMAL;
	hDMA1_P2M_stream.Init.Priority = DMA_PRIORITY_LOW;
	hDMA1_P2M_stream.Init.Request = DMA_REQUEST_LPUART1_RX;

	if(HAL_DMA_Init(&hDMA1_P2M_stream) != HAL_OK)
	{
		Error_Handler();
	}
